...
#define I(i)  (W[i] = htobe32(*(uint32_t*)(data + i * 4)))
...
static void sm3_transform(struct sm3_state *sctx, uint8_t const *data, uint32_t W[16])
{
...
static inline void sm3_block(struct sm3_state *sctx,
		uint8_t const *data, int blocks, uint32_t W[16])
{
	while (blocks--) {
...
void sm3_update(struct sm3_state *sctx, const uint8_t *data, unsigned int len)
{
...
void sm3_final(struct sm3_state *sctx, uint8_t *out)
{
	const int bit_offset = SM3_BLOCK_SIZE - sizeof(uint64_t);
	...
	*bits = htobe64(sctx->count << 3);
	sm3_block(sctx, sctx->buffer, 1, W);

	for (i = 0; i < 8; i++){
		//put_unaligned_be32(sctx->state[i], digest++);
		//zend_printf("state[%d] = %d \n", i, sctx->state[i]);
		*digest = htobe32(sctx->state[i]);
		digest++;
	}
